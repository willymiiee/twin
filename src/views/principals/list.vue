        ...
        <div class="card">
          <div class="card-body">
            <div class="row card-title mb-4">
              <div class="col">
                <h5>Daftar Principal</h5>
              </div>

              <div class="col">
                <router-link
                  class="btn btn-primary btn-sm float-right"
                  :to="{ name: 'principal-form', params: { id: 'add' } }"
                >Daftarkan Principal Baru</router-link>
              </div>
            </div>
                    ...
                    <tr v-for="d in datas" :key="d.id">
                      ...
                      <td class="pr-0 text-right">
                        <router-link
                          ...
                          :to="{ name: 'principal-form', params: { id: d.id } }"
                        >Detil</router-link>
                      </td>
                    </tr>
                  </tbody>
...
export default {
  ...
  methods: {
    getData () {
      let self = this
      self.$http
        .get('admin/principal?page=' + self.currentPage)
        .then(resp => {
          self.loaded = true
