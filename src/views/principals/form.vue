<template>
  <section class="principal-add">
    <div class="row">
      <div class="col-12 grid-margin">
        <div class="card">
          <div class="card-body">
            <h5 class="card-title mb-4">{{ this.label }} Principal</h5>
            ...
            <form v-if="loaded" class="forms-sample">
              ...
              <b-button
                ...
                variant="success"
                class="mr-2"
                @click.prevent="submitPrincipal"
              >{{ this.label }}</b-button>
              <b-button v-else disabled variant="success" class="mr-2">
                <i class="fa fa-spinner fa-pulse"></i>
              </b-button>
              <router-link :to="{ name: 'principal-list' }">
...
export default {
  ...
  methods: {
    ...
    getData() {
      let self = this
      self.$http
        .get('admin/principal/' + self.$route.params.id)
        .then(resp => {
    ...
    submitPrincipal() {
      let self = this
      const url =
        'admin/principal' + (self.$route.params.id !== 'add' ? '/' + self.$route.params.id : '')
      self.submit = true
      ...
      apiCall
        .then(resp => {
          self.error = {}
          self.$router.push({ name: 'principal-list' })
        })
        .catch(err => {
